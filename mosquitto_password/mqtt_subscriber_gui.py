...
class MQTTSubscriberGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("MQTT Subscriber GUI - Password")
        self.root.geometry("600x500")
        
        self.client = None
        self.is_connected = False
        print("[sub_gui_pw] App initialized")
        self.create_widgets()
    ...
    def connect_mqtt(self):
        ...
        print(f"[sub_gui_pw] toggle_connection -> is_connected={self.is_connected}, broker={broker}, port={self.port_var.get()}")
        try:
        ...
        try:
            print("[sub_gui_pw] Initializing MQTT client")
            ...
            print(f"[sub_gui_pw] Connecting to {broker}:{port}...")
            self.client.connect(broker, port, 60)
            print("[sub_gui_pw] Starting network loop")
            self.client.loop_start()
            
            self.btn_connect.config(state="disabled")
        except Exception as e:
            print(f"[sub_gui_pw] connect_mqtt error: {e}")
            messagebox.showerror("Error", f"Gagal connect: {e}")

    def disconnect_mqtt(self):
        if self.client:
            print("[sub_gui_pw] disconnect_mqtt called")
            self.client.disconnect()
            self.client.loop_stop()

    def on_connect(self, client, userdata, flags, reason_code, properties):
        print(f"[sub_gui_pw] on_connect called, reason_code={reason_code}")
        if reason_code == 0:
    ...
    def on_disconnect(self, client, userdata, flags, reason_code, properties):
        print(f"[sub_gui_pw] on_disconnect called, reason_code={reason_code}")
        self.is_connected = False
